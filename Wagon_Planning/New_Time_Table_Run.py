...
class NewTimeTable:
    def __init__(self, time_buckets, timetable_file, wagon_config, wagon_mapping):
                 # inventory_output_file,adjustments_output_file, wagon_configuration_output, num_weeks):
        self.num_weeks = 1
        # self.inventory_output = inventory_output_file
        # self.adjustments_output = adjustments_output_file
        ...
        # self.wagon_plan = wagon_config

        self.wagon_plan = self.platform_mapping(wagon_config)
        ...
        self.wagon_configuration_file = self.convert_train_services_dic_to_df()
    ...
    def initialise_train_services(self, timetable_file):
        ...
        train_services['DEP_TIME'] = pd.to_datetime(train_services['DEP_TIME'], dayfirst=True,
                                                    errors='coerce')
        train_services['ARR_TIME'] = pd.to_datetime(train_services['ARR_TIME'], dayfirst=True,
                                                    errors='coerce')
    ...
    def create_current_config(self):
        ...
        for train_number, leg_dictionary in self.train_services_dic.items():
            ...
            relevant_rows = (relevant_rows.groupby(["TRAIN_NAME", 'WAGON_CLASS', "LEG_NUM", "PLATFORM", "NUM_PLATFORMS",
                                                    "FAMILY", 'SIZE', 'ALL_EQUAL', 'CONFIGURATION'])["NUM_WAGONS"]
                             .sum().reset_index(name='NUM_WAGONS'))
    ...
    def create_terminals(self):
        ...
        for the_terminal in terminals_dict.values():
            the_terminal.wagon_inventory = {the_wagon: 0 for the_wagon in self.wagon_types}
    ...
    def balance_inventory(self):
        ...
        def update_inventory(terminal, wagon_config, adjustment, train_leg, train_number, leg_number):
            ...
            if NewTimeTable.is_within_time_bucket(
                ...
                for wagon_type, wagon in wagon_config.items():
                    wagon_inventory[terminal][wagon_type] += adjustment * wagon.num_wagons
            ...
            for train_number, the_leg in self.train_services_dic.items():
                for leg_number, train_leg in the_leg.items():
                    ...
                    update_inventory(train_leg.leg_destination_terminal, train_leg.wagon_configuration, 1,
                                     train_leg, train_number, leg_number)
        ...
        def convert_to_data_frame():
            ...
            return inventory_df, adjustment_df

        # Returns a tuple inventory_levels, adjustments
        return convert_to_data_frame()
